...
Also, there must be a shared network in the cloud that allows virtual machines to access hosts outside the cloud,
and possibly assign floating IPs to them, so that these VMs can be reached from outside the cloud. Let us call
this network the **provider** network.
...
the **fogbow-resource-allocation-service**, **fogbow-membership-service**, **fogbow-federated-network-service**,
and the **fogbow-gui**. We assume that the **fogbow-gui**, which provides a graphical user interface to Fogbow, will
be used only from inside the organization's private network. Users needing to use it outside the organization will
first need to setup a VPN. Let us assume that the CIDR of the organization's private network is **10.30.0.0/24**.
...
in the example shown in the Figure). You will also need this network ID, which can be seen by clicking
on the network name, and then on the "Overview" tab (see Figure below; in this example the ID is
**64ee4355-4d7f-4170-80b4-5e8348af6a61**).

![alt logo](../images/fogbow-ms/network-overview-screen.png "Network Overview")

Then, access the "Project/Compute/Access & Security" menu, and click on the "+ CREATE SECURITY GROUP" button to create
two new Security Groups, one for each the **dmz-host**, and another to the **internal-host**, as described below.
...
Then, click on the "MANAGE RULES" button, and multiple times on the "+ADD RULE" button to add the following rules:
...
After that, the **dmz-host-sg** Security Group should look like shown in the Figure below.

![alt logo](../images/fogbow-ms/dmz-host-sg-screen.png "Security Group of the dmz-host")

The 5222, 5347 and 5269 ports are used by the **messaging-service**, while the 8080 and the ranges 10000-11000 and
5000-6000 are used by the **fogbow-reverse-tunnel-service**. These range of ports are configured later on, so other
ranges can be used, if preferred. In the following, we will refer to the range 10000-11000 as the
**external-RTS-range**, and to the range **5000-6000** as the **internal-RTS-range**. Enabling ingress to
port 22 from the organization's private network allow the software to be installed in the **dmz-host** from a
machine in this network.
...
After that, the **internal-host-sg** Security Group should look like shown in the Figure below.

![alt_logo](../images/fogbow-ms/internal-host-sg-screen.png "Security Group of the internal-host")

The 80 port is used to connect to **fogbow-gui**, while the 22 port is used to install the necessary software
...
**dmz-host** will be created an will receive a private IP from the **provider** network. Let us
