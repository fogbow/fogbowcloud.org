Title: Install Fogbow Site
url: install-fogbow-site
save_as: install-fogbow-site.html
section: install
index: 1
...
3. Download the *fogbow-deploy* project:
...
The deployment machines can be either physical or virtual machines running Linux. Below we
provide detailed information for some possibilities on how to setup these machines.

- [Virtual machines on an Openstack cloud](2.1-openstack-deployment-machines-setup.md)
- [Virtual machines on a Cloudstack cloud](2.1-cloudstack-deployment-machines-setup.md)
...
For the purpose of the installation described in this guide, only some of the Required fields need to be edited,
since some of these fields can be automatically filled in by the installation scripts. In the following, we just
discuss the fields that the installation scripts cannot automatically gather, and, therefore, need to be filled
in by you.
...
Let us assume that the **xmpp_jid** of your site is **atm-prod.mycloud**. Then, the *intercomponent.conf* file would
look like (empty constants will be automatically configured by the installation script):

```bash
$ cat intercomponent.conf
# Required
xmpp_jid=atm-prod.mycloud
...
- [ATMOSPHERE project production federation]()
...
Your Fogbow site should be up now. Open a browser and point it to **fogbow**.**mydomain**, log in using the
appropriate credentials, and you are ready to manager resources in the cloud federation.